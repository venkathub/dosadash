...
Sources of truth are the Phase 2 admin settings (`Settings.business_hours`)
and `MenuItem.schedule` — both `{day: {start, end}}` with HH:MM windows
validated at write time (dosadash_shared.admin). Missing day key = closed /
not served that day; None/empty = always open / always served. Windows with
end < start span midnight.
