...
- [ ] Phase 4: Evals + LLMOps — IN PROGRESS on `phase/4-evals-llmops` (merged PRs #31–#33: golden set 18→80 tagged cases + coverage-floor gates, tool_correctness + guardrail_bypass suites + tone LLM-as-judge rubric v1 + one-pass run_live_evals.py, CI live merge gate @ order_accuracy ≥ 0.95 — first run caught 85%, calibrated data + prompt order_agent_v2 → 96.25% / tool 100% / 0 bypasses, ALL GATES PASS. In review: eval_runs table + CI ingest + admin scoreboard [migration f8c2d94a1b37]. Remaining: semantic cache, cost dashboard)
