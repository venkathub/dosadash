...
- [ ] Phase 4: Evals + LLMOps — CODE COMPLETE on `phase/4-evals-llmops`, pending Phase PR → main + deploy (PRs #31–#37: golden set 18→**150** tagged cases + coverage-floor gates; tool_correctness + guardrail_bypass suites + tone LLM-as-judge rubric v1; one-pass run_live_evals.py; CI live merge gate @ order_accuracy ≥ 0.95 — caught 85% then 92.67% pre-merge, fixed via data calibration + prompts order_agent_v2/v3 → final 150-case run **96.67% / tool 100% / 0 bypasses**; eval_runs table [migration f8c2d94a1b37] + CI ingest + admin Evals tab; semantic cache [Redis, cosine ≥ 0.95, cascade-flushed]; cost dashboard [Langfuse → ai → api → admin Costs tab]; secrets EVAL_INGEST_URL + INTERNAL_API_TOKEN configured. Known flakies to watch: ord-093/099/109/138/146)
