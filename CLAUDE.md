...
- [ ] Phase 4: Evals + LLMOps — CODE COMPLETE on `phase/4-evals-llmops`, pending Phase PR → main + deploy (PRs #31–#35 + costs PR: golden set 18→80 tagged cases + coverage-floor gates; tool_correctness + guardrail_bypass suites + tone LLM-as-judge rubric v1; one-pass run_live_evals.py; CI live merge gate @ order_accuracy ≥ 0.95 — first run caught 85%, calibrated data + prompt order_agent_v2 → 4 consecutive passes ≥95% / tool 100% / 0 bypasses; eval_runs table [migration f8c2d94a1b37] + CI ingest + admin Evals tab; semantic cache [Redis, cosine ≥ 0.95, cascade-flushed]; cost dashboard [Langfuse daily metrics → ai → api → admin Costs tab]; secrets EVAL_INGEST_URL + INTERNAL_API_TOKEN configured)
