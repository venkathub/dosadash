...
- [ ] Phase 2: Admin Backend I — IN PROGRESS (`phase/2-admin-backoffice`): DONE — menu ops (#13), settings + kitchen pause + staff RBAC + audit (#14), admin order mgmt + refunds (#15, migration c41f7a2d9b03), combos + ingredients + recipe mapping (#16), nutrition auto-enrichment (#17, migration e7b9c4d15a22 — first LLM code: litellm chain + Langfuse + `nutrition_v1` prompt + eval gates in CI); event cascade complete. REMAINING: admin web UI (`/admin` placeholder), business-hours/schedule enforcement follow-up; then Phase PR → main (needs OPENAI_API_KEY in prod .env for enrichment)
