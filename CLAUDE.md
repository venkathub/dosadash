...
- [ ] Phase 5: Classical ML + Admin II — IN PROGRESS (`phase/5-ml-reports`; PRs #40–#42 merged into phase branch): **done:** schema [migration a9e4b71c3d58: forecasts (unique item_id+date), customer_segments, orders.delivered_at; state machine stamps delivered_at on DELIVERED; datagen/seed emit delivery durations 18–90 min with peak/festival signal → ETA label] · ML dep extras [dosadash-ml \[forecast\]=pandas+xgboost / \[train\]=mlflow+sklearn; api \[worker\]=celery — api/bot images stay lean] · Celery worker+beat [compose `worker` (320M, same api image via UV_EXTRA_ARGS=--extra worker) + `redis-celery` (32mb noeviction — cache redis is allkeys-lru, unsafe broker); IST beat: hourly ops.heartbeat, 02:00 forecast.nightly_demand; total compose limits ≈2.35 GB] · XGBoost demand forecast [lag-7/14, ma_7, dow, festival_mult sharing datagen's category_multiplier + train↔score skew test; MLflow registry `dosadash-demand-forecast` @ champion alias, promote iff WAPE improves; **champion v1 WAPE 0.421 vs naive lag-7 0.555**; artifacts committed → /app/models; nightly recursive 14-day upsert into forecasts with model_version]. **Remaining:** ETA regression (label now exists) · Reports (sales, dish P&L, GST CSV) · CRM segments (RFM/churn scoring @ 03:00 → customer_segments) · forecast-vs-actual + anomaly flags (admin tab) · Text-to-SQL analytics copilot (read-only role, SQL validation guardrail, self-correction, charts)
- Notes for continuation: eval flakies to watch (ord-093/099/109/138/146 — wobble, not regressions; if a gate fails with only these, re-run before debugging) · tone judge is opt-in (`run_live_evals.py --with-tone`), not in the CI gate · GEMINI_API_KEY secret optional/unset (chain: OpenAI → Groq) · retrain/promote flow: `uv run python -m dosadash_ml.forecasting.train --synthetic` then commit `packages/ml/artifacts/` (mlflow.db/mlruns are gitignored) · worker logs: `docker compose logs worker` (hourly heartbeat = proof of life) · Phase 5 remaining work should reuse admin_costs/admin_evals router + tab patterns; CRM scoring belongs at 03:00 IST in the existing beat schedule
