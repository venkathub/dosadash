...
- [ ] Phase 6: Agents + MCP — NOT STARTED (`phase/6-agents-mcp`): inventory agent (stock vs forecast → draft PO → owner approval UI/Telegram → execute) · supplier invoice OCR (VLM structured extraction → PO matching → confidence-gated review queue → stock update) · wastage log · support agent (refund/status, policy guardrails, escalation inbox) · long-term agent memory ("my usual", episodic store) · MCP server (`get_menu`, `place_order`, `check_inventory`) — demo: Claude Desktop orders a dosa
